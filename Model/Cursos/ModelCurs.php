...
class ModelCurs {
    ...
    public function mostrarCurs($id_curs) {
        ...
        $check = $this->conn->prepare("SELECT * FROM continguts
        JOIN horarios ON continguts.id_contingut = horarios.id_contingut
        JOIN profesors ON continguts.id_profesor = profesors.id_profesor
        WHERE id_curs = ?");
        $check->bind_param("i", $id_curs);
        $check->execute();
        //Obtenim el resultat de la consulta
