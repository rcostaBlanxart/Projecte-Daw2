...
if ($count == 0) {
    ...
    $div_cursos = "";
    $total = 0; // variable para almacenar el total de los precios de los cursos reservados
    while($curs=$result->fetch_assoc()) {
        $nombreCurso = $curs["nom_curs"];
        $id_reserva = $curs["id_reserva"];
        $preu = $curs["preu"];
        $total += $preu; // acumulamos el precio del curso a la variable $total
    ...
    // imprimimos el total de los precios de los cursos reservados
